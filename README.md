> 在使用 WeFlow 和蜜语的时候，发现他们功能强大，更新很快，但交互逻辑复杂，细节体验比较糟糕，并且导出和分析功能杂糅。当用户的核心诉求仅仅是把微信中的各项数据导出时，缺少一个足够简单好用的工具。因此，我在前人项目基础之上，使用 Claude Code 和 Codex，对一些和导出无关的功能进行了删除，并且对整个导出功能进行了重新设计，让大家在使用导出时更方便，也更能知道哪些数据可以导出、值得导出、什么时候导出过。
