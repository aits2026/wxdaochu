...
export class CacheService {
  ...
  /**
   * 获取图片缓存目录（兼容旧的 VXdaochu/Images 路径）
   */
  private getImagesCachePaths(): string[] {
    ...
    paths.push(join(documentsPath, 'VXdaochu', 'Images'))
  ...
  async clearEmojis(): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const emojiPaths = [
        ...
        join(documentsPath, 'VXdaochu', 'Emojis'),
      ]
  ...
  async clearAll(): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (!existsSync(cachePath)) {
        // 同时检查旧的 VXdaochu 目录
        const documentsPath = app.getPath('documents')
        const oldVXdaochuDir = join(documentsPath, 'VXdaochu')
        if (existsSync(oldVXdaochuDir)) {
          rmSync(oldVXdaochuDir, { recursive: true, force: true })
        }
        return { success: true }
      }
  ...
  async getCacheSize(): Promise<{ 
    ...
    try {
      ...
      // 也检查旧的 VXdaochu 目录
      const oldEmojiPath = join(documentsPath, 'VXdaochu', 'Emojis')
      ...
      const oldVXdaochuDir = join(documentsPath, 'VXdaochu')
      if (existsSync(oldVXdaochuDir)) {
        databasesSize += this.getDatabaseFilesSize(oldVXdaochuDir)
      }
