...
class DataManagementService {
  ...
  async scanDatabases(): Promise<{ success: boolean; databases?: DatabaseFileInfo[]; error?: string }> {
    try {
      ...
      // 获取缓存目录（优先使用配置的路径）
      let vxdaochuDir = this.configService.get('cachePath')
      if (!vxdaochuDir) {
        vxdaochuDir = this.getDefaultCachePath()
      }
      ...
      if (lastPart === 'db_storage') {
        ...
        await this.scanDbStorageDirectory(dbPath, accountName, vxdaochuDir, databases)
      } else {
        // 只扫描配置的 wxid 目录
        ...
        if (fs.existsSync(dbStoragePath)) {
          await this.scanDbStorageDirectory(dbStoragePath, cleanedAccountName, vxdaochuDir, databases)
        } else {
  ...
  private async scanDbStorageDirectory(
    ...
    for (const filePath of dbFiles) {
      ...
      const decryptedPath = path.join(vxdaochuDir, accountName, decryptedFileName)
