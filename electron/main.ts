...
function registerIpcHandlers() {
  ...
  ipcMain.handle('dbpath:getBestCachePath', async () => {
    ...
    for (const drive of drives) {
      ...
      if (existsSync(drivePath)) {
        const cachePath = join(drivePath, 'VXdaochuDB')
  ...
  ipcMain.handle('export:getLatestEmojiExportTimes', (_, sessionUsernames: string[]) => {
    return exportRecordService.getLatestEmojiExportTimes(Array.isArray(sessionUsernames) ? sessionUsernames : [])
  })

  ipcMain.handle('export:getLatestImageExportTimes', (_, sessionUsernames: string[]) => {
    return exportRecordService.getLatestImageExportTimes(Array.isArray(sessionUsernames) ? sessionUsernames : [])
  })

  ipcMain.handle('export:getLatestVideoExportTimes', (_, sessionUsernames: string[]) => {
    return exportRecordService.getLatestVideoExportTimes(Array.isArray(sessionUsernames) ? sessionUsernames : [])
  })
  ...
  ipcMain.handle(
    ...
    (
      ...
      exportRecordService.saveRecord(sessionUsername, format, messageCount, outputDir, outputTargetType, exportEmojisIncluded, extra)
    }
  )
